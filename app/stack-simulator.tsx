...
import BitcoinStackBackground from "./stack-background"
...
const opcodes: Opcode[] = [
    ...
    },
    {
        name: "OP_EQUALVERIFY",
        execute: (stack) => {
            const newStack = opcodes.find(op => op.name === "OP_EQUAL")!.execute(stack)
    ...
    },
    {
        ...
        execute: (stack) => {
            ...
            try {
                ...
                const valid = keyPair.verify(sha256(Buffer.from("message to sign")), signature)
                return [...stack, valid]
            } catch (error) {
            ...
            try {
                ...
                for (const signature of signatures) {
                    for (const pubKey of pubKeys) {
                        const keyPair = bitcoin.ECPair.fromPublicKey(pubKey)
                        if (keyPair.verify(sha256(message), signature)) {
                            validCount++
                            break
                        }
                    }
...
const StackSimulator = ({ showOperations }: StackSimulatorProps) => {
    ...
    return (
        ...
        <BitcoinStackBackground></BitcoinStackBackground>
